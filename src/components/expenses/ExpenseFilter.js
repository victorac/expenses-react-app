...
function ExpenseFilter({ expenses, onSelectYear }) {
  ...
  if (expenses.length > 0) {
    totalAmount = expenses.map(el => el.amount).reduce(
  ...
  expenses.forEach((element) => {
    monthData[element.date.getMonth()].percentage += Number(element.amount) / totalAmount * 100;
  });
  ...
  return (
    <div className="flex flex-col gap-4 w-[50rem] max-w-[95%] mx-auto">
      ...
      <Card className="flex flex-row gap-4 text-sm bg-orange-200 px-3 py-2">
        {monthData.map(data => <MonthBar key={data.month} month={data.month} percentage={data.percentage}/>)}
      </Card>
    </div>
  );
}
...
function MonthBar({ month, percentage }) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center gap-1">
      <div
        className="h-[100px] w-4 rounded-md"
        style={backgrounColor}
      ></div>
      <span>{month}</span>
    </div>
  );
